# Source: https://www.dropbox.com/s/tp66exudntpwdim/Euphoria%20-%20Custom%20Gauge%20-%2012.rb
...
#==============================================================================                           
#Instructions: Create you're own gauge! In the editable region you can name the
#              gauge, set it's max value, current value, colors, etc. To add to
#              the gauge outside of battle you can use the script call:
#
#              $game_actors[x].add_gauge(y)  x = actor number, y = amount to add
#
#              Alternately you can subtract with the script call:
#
#              $game_actors[x].sub_gauge(y)
#
#              To use the values in conditional branches you may check the value
#              by typing in these:
#
#              $game_actors[x].gauge == y    x = actor, y  = amount to check for
#              $game_actors[x].gauge <= y
#              $game_actors[x].gauge >= y
#
#              If you wish to use the gauge in battle TP MUST BE DISABLED and
#              you can set skills to use up your gauges value by note tagging a
#              skill with:
#
...
module Euphoria
  module Custom_Gauge
    ...
    GAUGE_NAME      = "Rage"     #Set The Gauges Name (5 Characters MAX)
    ...
    GAUGE_COLOR2    = 1          #Set The Gauges Right Side Color (1-32)
    ...
    AT_TAKE_DAMAGE  = 10         #Amount Gained/Lost When Taking Damage
     
    AT_HEAL_TARGET  = 0          #Amount Gained/Lost When Healing Target
     
    AT_KILL         = 20          #Amount Gained/Lost When Killing Target
...
class Window_Base < Window
  ...
  def draw_actor_gauge(actor, x, y, width = 124, extension = 50)	
    draw_gauge(x, y, width + extension, actor.gauge / Euphoria::Custom_Gauge::GAUGE_MAX.to_f, text_color(Euphoria::Custom_Gauge::GAUGE_COLOR1), text_color(Euphoria::Custom_Gauge::GAUGE_COLOR2))
    change_color(system_color)
    draw_text(x, y, 100, line_height, Euphoria::Custom_Gauge::GAUGE_NAME)
    draw_current_and_max_values(x, y, width + extension, actor.gauge, Euphoria::Custom_Gauge::GAUGE_MAX, text_color(0), text_color(0))
  end
  
end
#===============================================================================
# Setup Gauge Functions <><><><><><><><><><><><><><><><><><><><><><><><><><><><>
#===============================================================================  
